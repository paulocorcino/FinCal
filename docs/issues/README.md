...
- **UI/UX**: toda tela obedece o [`docs/ui-ux.md`](../ui-ux.md) — casca de navegação, tabela semântica (cores de Tipo/Status/atrasado), componentes compartilhados (`CurrencyInput`, `DateField`, `EmptyState`, modal de Lançamento), casca do Assistente e padrões de confirmação. Não freelance a frontend: se um padrão existe lá, reuse.
